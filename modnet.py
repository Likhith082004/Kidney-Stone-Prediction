...
def train_mobilenet(dataloaders, dataset_sizes, device, num_epochs=1):
    model = MobileNetKidneyClassifier().to(device)
...
def plot_training_curves(history, save_path='mobilenet_training_curves.png'):
    plt.figure(figsize=(12, 4))

    plt.subplot(1, 2, 1)
    plt.plot(history['train_losses'], label='Train Loss')
    plt.plot(history['test_losses'], label='Test Loss')
    plt.title('MobileNet - Training and Testing Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    ...
    plt.title('MobileNet - Training and Testing Accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.legend()
...
def main():
    ...
    # UPDATED PATH – SAME AS VGG16 & INCEPTION
    data_dir = "kidney_dataset"

    dataloaders, dataset_sizes = create_datasets(data_dir)

    model, history = train_mobilenet(dataloaders, dataset_sizes, device)
    torch.save(model.state_dict(), 'mobilenet_kidney_classifier.pth')
